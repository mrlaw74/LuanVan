#include "binSceneMaker.h"

BinSceneMaker::BinSceneMaker() {}

void BinSceneMaker::doWork(const QString &parameter)
{
	QString result;
	if (parameter == "makeBinScene")
	{
		makeBinScene();
		result = "makeBinSceneSuccess";
		emit resultReady(result);
		return;
	}
}

void BinSceneMaker::loadModel(pcl::PointCloud<pcl::PointXYZ>::Ptr& model_)
{
	model = model_->makeShared();
	//std::cout << "Model Simulation Loaded!" << std::endl;
	//std::cout << model->points.size() << std::endl;
}

bool BinSceneMaker::makeBinScene()
{
	//Clear previous cloud
	simulationViewer->removeAllPointClouds();
	simulationViewer->removeAllShapes();
	...
	//
	pcl::copyPointCloud(*model, *model_load);
	...
	std::cout << "PPF MEAM Estimation: Finished." << std::endl;
	...
	{
		simulationViewer->addPointCloud(cup_ground, "cup_ground");
	}
		...
		if (cnt > setting.falling_interval)
		{
			...
			if (now_model_num < setting.model_num)
			{
				...
				if (setting.visualization)
				{
					...
					simulationViewer->addPointCloud(model, cloud_name);
				}
				++now_model_num;
		...
		for (int idx = 0; idx < dynamicsWorld->getNumCollisionObjects(); idx++)
		{
			...
			if (body && body->getMotionState() && body != cup_body)
			{
				if (setting.visualization)
				{
					...
					simulationViewer->updatePointCloudPose(cloud_name, Eigen::Affine3f(Eigen::Map<Eigen::Matrix4f>(mat_v)));
				}
			}
		}
	...
	save_cloud.reset(new pcl::PointCloud<pcl::PointXYZ>());
	...
	simulationViewer->setCameraPosition(0, 0, 0,      0, 0, 0.4,     0, -1, 0);
	...
	simulationViewer->addPointCloud(save_cloud, "save_cloud");
	emit updateUiFromWorker("simulationViewer");
}