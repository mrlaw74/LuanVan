...
namespace pcl
{
    ...
    public:
      ...
      struct HashKeyStruct : public std::pair <int, std::pair <int, std::pair <int, int> > >
      {
		...
		std::size_t operator()(const HashKeyStruct& s) const noexcept
        {
            ...
            return h1 ^ (h2 << 1) ^ (h3 << 2) ^ (h4 << 3); // ^: Khac la 1, giong la 0
			// << Toan tu dich trai, gia tri toan hang trai duoc dich chuyen sang trai
			// A = 0011 1100 (60), A <<2 --> 11110000(240)
        }
      };
  ...
  class MyPPFRegistration : public Registration<PointSource, PointTarget>
  {
    public:
	  ...
	  inline void
		  setFullModel(pcl::PointCloud<pcl::PointXYZ>::Ptr& full_model) { full_model_ = full_model->makeShared(); }

	  inline void
      setLvoxel (double Lvoxel_input) { Lvoxel = Lvoxel_input; }

	  inline void
		  setModelSamplingRadius(double rad) { samp_rad = rad; }
    ...
    private:
	  ...
	  pcl::PointCloud<pcl::PointXYZ>::Ptr full_model_;
	...
	typename pcl::MyPPFRegistration<PointSource, PointTarget>::PoseWithVotesList &result)
{
	...
	{
		if (poses[pose_index].votes < 0.5*poses[0].votes || pose_index > 5)
		{
	...
	while (poses.size() > 2)
	{
		...
		if (poses[0].votes < 0.6 * result[0].votes)
			return;
	}
